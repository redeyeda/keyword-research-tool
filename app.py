...
with st.sidebar:
    ...
    # ── 키워드 확장용 연관단어
    st.markdown("**🎯 키워드 확장 연관단어**")
    st.caption("입력한 단어로 추가 키워드를 검색하고 점수에도 반영됩니다.")
    keyword_expand_input = st.text_area(
        "확장 단어",
        placeholder="예: 피부장벽, 건조한피부, 민감성",
        height=70,
        key="keyword_expand",
    ...
    # ── 업종 프리셋 + 쇼핑 점수 전용
    st.markdown("**🛒 쇼핑 점수 업종 프리셋**")
    st.caption("선택하면 쇼핑 점수 계산에 반영됩니다.")
    ...
    st.markdown("**➕ 쇼핑 점수 전용 단어 추가**")
    st.caption("쇼핑 점수 계산에만 사용됩니다. (키워드 검색 미포함)")
...
def verify_keywords_volume(keywords, api_key, secret_key, customer_id):
    """
    3단계: 키워드 검색량 검증
    수집된 키워드를 네이버 광고 API에 단일 조회
    → 실제 검색량 있는 키워드만 반환
    """
    ...
    for kw in keywords:
        ...
        if _is_valid(kw_clean):
            query_kws.append(kw_clean)
        else:
            # 공백 포함이면 각 단어 개별 추가
            for w in kw_clean.split():
                if _is_valid(w) and w not in query_kws:
                    query_kws.append(w)

    query_kws = list(dict.fromkeys(query_kws))
    results   = []
...
def get_naver_suggest_multi(keyword):
    """
    네이버 자동완성 다중 확장
    메인 키워드 + 조합어로 여러 번 자동완성 조회
    → 실제 검색 패턴 기반 롱테일 수집
    """
    # 검색 의도 조합어
    intent_words = [
    ...
    for intent in intent_words[:8]:  # 과도한 API 호출 방지
        query = f"{base} {intent}".strip() if intent else base
        try:
            r = requests.get(
...
def get_claude_longtail(main_keyword, api_key, naver_samples=None, google_samples=None):
    """
    Claude API — 실제 자동완성 패턴 기반으로 롱테일 확장
    naver_samples, google_samples: 1단계에서 수집한 실제 검색어 샘플
    """
    try:
        ...
        # 실제 검색 패턴 샘플 포함
        naver_sample_str  = ", ".join((naver_samples  or [])[:10])
        google_sample_str = ", ".join((google_samples or [])[:10])

        prompt = (
            f"Main keyword: {main_keyword}\n"
            f"Real Naver search patterns found: {naver_sample_str}\n"
            f"Real Google search patterns found: {google_sample_str}\n\n"
            "Based on these REAL search patterns, generate more similar longtail keywords.\n"
            "Rules:\n"
            "- Must be similar style to the real patterns above\n"
            "- Must contain the main keyword or its components\n"
            "- Naver: 30 keywords, informational intent (추천/후기/방법/효과/순위/비교)\n"
            "- Google: 30 keywords, question/informational style, longer phrases\n"
            "- All keywords in Korean\n"
            "Respond ONLY with JSON:\n"
            '{"naver_keywords":["kw1",...30],"google_keywords":["kw1",...30]}'
        )
        ...
        if r.status_code == 200:
            ...
            st.warning(f"Claude API [{r.status_code}]: {r.text[:200]}")
    except Exception as e:
...
if run_btn:
    ...
    # 키워드 확장 연관단어 (검색 + 점수 모두 반영)
    expand_raw = st.session_state.get("keyword_expand", "")
    expand_kws = [w.strip() for w in expand_raw.replace("，",",").split(",")
                 if w.strip() and len(w.strip()) >= 2] if expand_raw else []

    # 쇼핑 점수 전용 커스텀 단어
    custom_kws = [w.strip() for w in custom_raw.replace("，",",").split(",")
                 if w.strip() and len(w.strip()) >= 2] if custom_raw else []
    preset_kws = [w for w in preset_words if w and len(w) >= 2]

    # extra_kws = 확장 연관단어 + 쇼핑 프리셋 (쇼핑 점수용)
    extra_kws  = list(dict.fromkeys(expand_kws + custom_kws + preset_kws))

    if expand_kws:
        st.info(f"🎯 키워드 확장 연관단어: {', '.join(expand_kws)}")

    # ① 자동완성 수집
    log.info("① 자동완성 수집 중...")
    naver_sug  = get_naver_suggest_multi(main_kw)
    google_sug = get_google_suggest(main_kw)
    prog.progress(18, text=f"① 자동완성 {len(naver_sug)+len(google_sug)}개")

    # ② Claude 확장
    log.info("② Claude AI 확장 중...")
    ...
    if claude_api_key:
        ...
        prog.progress(32, text=f"② AI 확장 {len(claude_naver)+len(claude_google)}개")
    else:
        prog.progress(32, text="② Claude 없음")

    all_sug = naver_sug + google_sug + claude_naver + claude_google

    # ── 메인 힌트 구성 (메인키워드 관련)
    main_hints = []
    main_hints.append(main_kw.replace(" ", ""))  # 공백제거 버전
    for w in main_words:
        if w not in main_hints:
            main_hints.append(w)
    for kw in all_sug:
        kw_ns = kw.replace(" ", "")
        if all(w in kw_ns for w in main_words) and kw_ns not in main_hints:
            main_hints.append(kw_ns)
    main_hints = list(dict.fromkeys(main_hints))

    # ── 확장 힌트 구성 (키워드 확장 연관단어만 사용)
    extra_hints = []
    for w in expand_kws:
        w_ns = w.replace(" ", "")
        if w_ns and w_ns not in main_hints and w_ns not in extra_hints:
            extra_hints.append(w_ns)

    prog.progress(35, text=f"③ 힌트 준비 — 메인 {len(main_hints)}개 / 연관 {len(extra_hints)}개")

    # ③ 네이버 광고 API — 메인 힌트 조회
    log.info(f"③ 메인 키워드 조회 중... ({len(main_hints)}개)")
    main_stats = get_naver_keyword_stats(
        main_hints, naver_api_key, naver_secret_key, naver_customer_id
    )

    # ④ 네이버 광고 API — 연관단어 힌트 조회
    extra_stats = []
    if extra_hints:
        log.info(f"④ 연관단어 조회 중... ({len(extra_hints)}개)")
        extra_stats = get_naver_keyword_stats(
            extra_hints, naver_api_key, naver_secret_key, naver_customer_id
        )

    prog.progress(65, text=f"③④ 완료 — 메인 {len(main_stats)}개 / 연관 {len(extra_stats)}개")

    if not main_stats and not extra_stats:
        st.error("⛔ 네이버 광고 API 데이터가 없습니다.")
        st.stop()
    ...
    main_stats  = dedup(main_stats)
    extra_stats = dedup(extra_stats)

    # ⑤ 메인 결과 — AND 필터 (메인키워드 모든 단어 포함)
    if len(main_words) >= 2:
        and_match = [k for k in main_stats
                    if all(w in k.get("relKeyword","") for w in main_words)]
        or_match  = [k for k in main_stats
                    if main_words[0] in k.get("relKeyword","")]
        if len(and_match) >= 3:
            main_stats = and_match
            st.caption(f"🎯 메인 AND필터({'+'.join(main_words)}): {len(main_stats)}개")
        elif len(or_match) >= 3:
            main_stats = or_match
            st.caption(f"🎯 메인 첫단어필터({main_words[0]}): {len(main_stats)}개")
        else:
            st.warning(f"⚠️ '{main_kw}' 관련 키워드가 네이버 DB에 부족합니다.")
    elif len(main_words) == 1:
        relevant = [k for k in main_stats if main_words[0] in k.get("relKeyword","")]
        if len(relevant) >= 3:
            main_stats = relevant

    # ⑥ 연관단어 결과 — 메인키워드와 중복 제거
    main_kw_set = {k.get("relKeyword","") for k in main_stats}
    extra_stats = [k for k in extra_stats
                  if k.get("relKeyword","") not in main_kw_set]
    ...
    # 전체 합산
    naver_stats = main_stats + extra_stats
    ...
    # 메인/연관단어 분리
    df_main  = df_all[df_all["키워드"].apply(
        lambda kw: all(w in kw for w in [w for w in main_keyword.strip().split() if len(w)>=2])
        if len([w for w in main_keyword.strip().split() if len(w)>=2]) >= 2
        else True
    )].copy()
    df_extra = df_all[~df_all.index.isin(df_main.index)].copy()
    ...
    with tab3:
        st.caption(f"메인키워드 '{main_keyword}' 포함 키워드 — {len(df_main)}개")
        st.dataframe(df_main.sort_values("네이버블로그_적합도",ascending=False).reset_index(drop=True),
                    use_container_width=True, height=420)
    with tab4:
        st.caption(f"연관단어 기반 확장 키워드 — {len(df_extra)}개")
        st.dataframe(df_extra.sort_values("네이버블로그_적합도",ascending=False).reset_index(drop=True),
                    use_container_width=True, height=420)
    with tab5: st.dataframe(df_shop,   use_container_width=True, height=420)
    with tab6: st.dataframe(df_trend,  use_container_width=True, height=420)
    with tab7: st.dataframe(df_all.sort_values("월간검색(합계)",ascending=False).reset_index(drop=True),
                            use_container_width=True, height=420)
