...
if run_btn:
    ...
    # ① 힌트 구성 — 메인키워드 우선, 단어분리 보조
    log.info("① 힌트 키워드 구성 중...")

    # 메인키워드 전체 (공백제거 버전도 추가)
    hint_set = []
    hint_set.append(main_kw.replace(" ", ""))  # "세라마이드토너"
    for w in main_words:
        hint_set.append(w)  # "세라마이드", "토너"

    # 네이버 자동완성 (메인키워드 전체로 조회)
    naver_sug = get_naver_suggest_multi(main_kw)
    google_sug = get_google_suggest(main_kw)

    prog.progress(18, text=f"① 자동완성 {len(naver_sug)+len(google_sug)}개 수집")
    ...
    # 자동완성+AI 결과에서 메인키워드 모든 단어 포함된 것만 힌트에 추가
    all_sug = naver_sug + google_sug + claude_naver + claude_google
    for kw in all_sug:
        kw_nospace = kw.replace(" ", "")
        # 메인키워드 모든 단어가 포함된 키워드만 힌트로 추가
        if all(w in kw_nospace for w in main_words):
            hint_kw = kw_nospace  # 공백 제거해서 API에 전송
            if hint_kw not in hint_set and len(hint_kw) >= 2:
                hint_set.append(hint_kw)

    hint_set = list(dict.fromkeys(hint_set))
    prog.progress(35, text=f"③ 힌트 {len(hint_set)}개 준비")

    # ③ 네이버 광고 API 조회
    log.info(f"③ 네이버 광고 API 조회 중... ({len(hint_set)}개)")
    ...
    # ④ 관련성 필터 — 메인키워드 모든 단어 포함(AND 조건)
    if len(main_words) >= 2:
        # 2단어 이상: 모든 단어 포함 (AND)
        and_match = [k for k in naver_stats
                    if all(w in k.get("relKeyword","") for w in main_words)]
        # AND 결과 부족하면 첫 단어만 필수
        or_match  = [k for k in naver_stats
                    if main_words[0] in k.get("relKeyword","")]

        if len(and_match) >= 3:
            naver_stats = and_match
            st.caption(f"🎯 AND 필터({'+'.join(main_words)}): {len(naver_stats)}개")
        elif len(or_match) >= 3:
            naver_stats = or_match
            st.caption(f"🎯 첫단어 필터({main_words[0]}): {len(naver_stats)}개")
        else:
            st.warning(f"⚠️ '{main_kw}' 관련 키워드가 네이버 DB에 부족합니다. "
                      f"단어 수를 줄이거나 더 일반적인 키워드로 시도해보세요.")
